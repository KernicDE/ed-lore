---
uuid: cb000662-d78f-5013-89e5-b8375bd4b9d8
title: Pleiades Rescue Operation Concludes
slug: pleiades_rescue_operation_concludes
date: '3304-08-18'
source: GitHub
summary: Aegis Research announces the successful conclusion of its Pleiades rescue
  operation. Hundreds of pilots delivered stranded survivors and commodities to Donar's
  Oak. Superintendent Marla Poulsen reports a fantastic number of rescued survivors
  and sufficient food stocks to care for them before moving to dedicated refugee centres.
player_impact: Contributing pilots can collect rewards from Donar's Oak in the Pleiades
  Sector IR-W d1-55 system.
persons:
- Marla Poulsen
groups:
- Aegis
- Aegis Research
locations:
- Pleiades Sector IR-W d1-55
- Donar's Oak
topics:
- humanitarian
- war
modern_impact: The successful rescue operation demonstrates the galactic community's
  capacity for humanitarian response even in active war zones, saving hundreds of
  civilian lives.
legacy_weight: 3
significance: medium
related_uuids:
- 1afd3193-1f8d-51ed-b04a-46317a5e1cae
---


A spokesperson for The Aegis Research has announced that Its rescue operation in the Pleiades has been enthusiastically assisted by the galactic community. Hundreds of pilots supported the operation by delivering stranded survivors and commodities to Donar's Oak in the Pleiades Sector IR-W d1-55 system. 

Superintendent Marla Poulsen of Aegis Research released The following statement: 

“The number of rescued survivors we have received is truly fantastic. We have also received enough food stocks to ensure that they’re well taken care of, before moving them to dedicated refugee centres.” 

Pilots who contributed to the initiative can now collect their rewards from Donar's Oak in the Pleiades Sector IR-W d1-55 system.
