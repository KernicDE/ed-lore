---
uuid: b1e570eb-029f-5cad-9996-9d7bb444f78f
title: Secure Resource Sites Declared in Dhan
slug: secure_resource_sites_declared_in_dhan
date: '3311-04-25'
source: API
summary: Combined security forces and independent pilots eradicate pirates from the
  Dhan system, securing its planetary rings. Gutamaya observers are reportedly 'very
  impressed' by the Corsair's performance.
player_impact: Independent pilots eliminated pirates and collected bounties to secure
  Dhan's resource sites, with participants collecting rewards from Kidd Station.
persons:
- Harald Gomes
topics:
- crime
- piracy
technologies:
- Corsair
modern_impact: Gomes's blunt characterisation of pirates as 'too foolish to give up
  a life on the wrong side of the law' reflects a pragmatic post-war security mindset.
  Gutamaya's reported satisfaction with Corsair performance in anti-piracy operations
  validates the ship's combat credentials, supporting its marketing as a multipurpose
  vessel.
legacy_weight: 2
significance: low
related_uuids:
- a633e23e-d7d8-53b4-ae7f-dedcfa4afbdd
---


Independent pilots have helped eradicate large numbers of pirates in the Dhan system.
Following an unusually high increase in piracy across the system's many planetary rings, local security fleets and skilled freelancers worked to eliminate pirates and show criminals that it would be better for them to relocate.
"Pirates are too foolish to give up a life on the wrong side of the law," said Dhan Empire League head of security Harald Gomes. "It's all they know, and all they're good for. But while we can't stamp out crime forever, we can at least send the message that Dhan isn't an easy target. Attack our mining craft at your peril, ya lugheads!"
Gutamaya, thought to have deployed observers to monitor use of its recently released Corsair craft during the conflict, is said to have been 'very impressed' by its craft's performance and the anti-piracy response force in general.
Participants that registered for the conflict can collect their rewards from Kidd Station.
