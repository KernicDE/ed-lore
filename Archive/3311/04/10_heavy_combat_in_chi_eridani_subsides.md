---
uuid: 4427945c-0535-54a9-8d6a-d3e86f8ebba7
title: Heavy Combat in Chi Eridani Subsides
slug: heavy_combat_in_chi_eridani_subsides
date: '3311-04-10'
source: API
summary: The proxy conflict between Jerome Archer and Denton Patreus supporters in
  Chi Eridani ends with an Archer victory.
player_impact: Pilots fought for both Jerome Archer and Denton Patreus in Chi Eridani,
  with Archer's supporters claiming victory and collecting rewards at Steve Masters
  starport.
persons:
- Jerome Archer
- Denton Patreus
topics:
- war
technologies: []
modern_impact: Archer's spokesperson's claim — 'this outcome proves the strong influence
  and military might of Jerome Archer's supporters in Chi Eridani' — frames the victory
  as proof of Federal strength in a traditionally contested system. The resolution
  without superpower escalation demonstrates that proxy wars remain contained, but
  the pattern of Imperial agents sparking Federal infighting establishes a playbook
  that will repeat.
legacy_weight: 2
significance: low
related_uuids:
- 69d27992-0ea2-58ac-8042-7cb0aecdc558
---


A proxy conflict between supporters of Jerome Archer and Denton Patreus appears to be slowing down after several days of sustained fighting.
Battlefield reporters for the Chi Eridani Herald state that the Jerome Archer group emerged victorious, scoring a higher number of confirmed victories in the area.
A spokesperson for Jerome Archer provided the following to the Herald:
"It's obvious that this outcome proves the strong influence and military might of Jerome Archer's supporters in Chi Eridani. Hopefully this leads to a widespread acceptance of the benefits and protection that Archer's leadership can bring to the system."
Those who took part in the conflict for either side can now collect their rewards.
Jerome Archer supporters can collect their rewards at Steve Masters starport.
Denton Patreus supporters can collect their rewards at the Evanescent Dusk megaship.
